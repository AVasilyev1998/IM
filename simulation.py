...
if __name__ == '__main__':
    ...
    print(sys.argv)
    params = [
                float(sys.argv[i]) if i == 3 else int(sys.argv[i])
                for i in range(1, len(sys.argv))
            ]
    if len(params) == 6:
        FILMS_COUNT = params[0]  # 7
        HALLS_COUNT = params[1]
        CLIENT_GEN_TIMEOUT = params[2]
        CLIENT_COUNT = params[3]
        TICKET_SHOP_CAPACITY = params[4]
        FOOD_SHOP_CAPACITY = params[5]
    else:
        FILMS_COUNT = 7
        HALLS_COUNT = 5
